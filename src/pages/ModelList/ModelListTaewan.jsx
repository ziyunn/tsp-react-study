...
import { Title3, Body1, Body3, calcRem, Body2 } from "../../style/font";
...
const listItems = ({ model, modelFn, modelList }) => {
  ...
  return (
    <ul>
      <InfiniteScroll
        next={modelFn}
        hasMore={modelListTotalCnt > modelList.length}
            ...
            return (
              <ListItem key={idx}>
                <Link to={`/modelView/${item.categoryCd}/${item.idx}`}>
                  <img
                    className="model-item__img"
                    src={`${process.env.REACT_APP_ADMIN_URL}${item.modelImage[0].fileMask}`}
                    alt=""
                  />
                  <Body1 weight="700" className="model-item__title">
                    {item.modelEngName}
                  </Body1>
...
const ModelList = () => {
  ...
  const listTab = () => {
    return (
      <ListTab>
        {Constants.LISTTAP.map((item, idx) => {
          return (
            <Body2 key={item.KEY} as="li">
  ...
  return (
    <>
      <ListWrap>
        <Title3 className="list-title font-abril" color={white}>
          {listTitle()}
        </Title3>
        {category === "3" ? listTab() : ""}
        {listItems({
